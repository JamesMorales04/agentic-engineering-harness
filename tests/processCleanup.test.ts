...
describe("process cleanup", () => {
  it("terminates descendants when a shell command times out", async () => {
    ...
    try {
      const result = await runProcess(`${shellQuote(process.execPath)} -e ${shellQuote(script)} ${shellQuote(pidFile)}`, { cwd: root, timeoutMs: 500 });
  ...
  it("settles when a detached descendant retains the helper stdio pipes", async () => {
    ...
    try {
      ...
      const result = await runProcess(`${shellQuote(process.execPath)} -e ${shellQuote(script)} ${shellQuote(pidFile)}`, { cwd: root, timeoutMs: 250 });
