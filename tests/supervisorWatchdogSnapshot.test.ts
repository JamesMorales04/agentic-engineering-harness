...
async function stalledOperation(): Promise<string> {
  ...
  await patchOperationMetadata(root, record.id, { lastProgressAt: new Date(Date.now() - 300_000).toISOString() });
  return root;
}

describe("snapshot-driven supervisor watchdog", () => {
  it("does not wake any LLM while an unresolved child runtime is still active", async () => {
    const root = await stalledOperation();
    const dispatch = vi.fn();
    const decision = await runOperationLivenessCheck(root, config, "AUDIT-WATCH", {
      dispatch: dispatch as never,
      inspect: vi.fn(async (_root: string, id: string) => ({ id, status: id === "reviewer-1" ? "running" : "idle" })) as never,
  ...
  it("gives an idle supervisor an authoritative snapshot instead of asking it to rediscover state", async () => {
    const root = await stalledOperation();
    const dispatch = vi.fn(async (_root: string, id: string) => ({ id, exitCode: 0, stdout: "", stderr: "", status: "working", transport: "sdk" as const }));
    await runOperationLivenessCheck(root, config, "AUDIT-WATCH", {
      dispatch: dispatch as never,
      inspect: vi.fn(async (_root: string, id: string) => ({ id, status: "idle" })) as never,
      trace: vi.fn(async () => undefined) as never,
    ...
    expect(prompt).toContain("Deterministic watchdog snapshot (authoritative for this wake)");
    expect(prompt).toContain('"logicalAgent":"test-reviewer"');
    expect(prompt).toContain("Do not run shell commands");
    expect(prompt).toContain("controller already performed runtime inspection");
  });
});
