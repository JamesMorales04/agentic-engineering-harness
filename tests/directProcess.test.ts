...
describe("direct worker process boundary", () => {
  ...
  it("does not leak operation state redirection into repository commands", async () => {
    ...
    try {
      const result = await runProcess(
        `${process.execPath} -e ${JSON.stringify("process.stdout.write(JSON.stringify({id:process.env.AEH_OPERATION_ID,root:process.env.AEH_CONTROL_ROOT,redirect:process.env.AEH_OPERATION_STATE_REDIRECT}))")}`,
  ...
  it("does not leak managed-agent identity into repository commands", async () => {
    ...
    try {
      const result = await runProcess(
        `${process.execPath} -e ${JSON.stringify(`process.stdout.write(JSON.stringify(${JSON.stringify(names)}.filter((name)=>process.env[name] !== undefined)))`)}`,
        { cwd: process.cwd(), timeoutMs: 2_000 }
      );
      expect(result.exitCode).toBe(0);
      expect(JSON.parse(result.stdout)).toEqual([]);
  ...
  it("settles after a timeout even when a detached descendant retains stdio", async () => {
    ...
    try {
      ...
      const result = await runDirectWorkerProcess(process.execPath, ["-e", script, pidFile], { version: 1, project: { name: "direct-process-test" } }, { cwd: root, timeoutMs: 100 });
