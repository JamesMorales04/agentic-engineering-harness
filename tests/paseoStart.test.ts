...
const config: HarnessProjectConfig = {
  ...
  orchestration: {
    provider: "paseo",
    interactive: { autoSetup: true, webUi: true, leadAgent: "lead", reuseSession: true, stateDir: ".harness/paseo", title: "AEH Lead" }
  }
};
...
function topology(): ResolvedAgentTopology {
  ...
  return {
    ...
    profile: "balanced",
    skillRoots: [],
    runtimes: { codex: { adapter: "codex", paseoProvider: "codex", capabilities: { sessions: true } } },
    models: { brain: model },
    agents: {
      lead: {
        name: "lead",
        ...
        runtime: { name: "codex", adapter: "codex", paseoProvider: "codex", capabilities: { sessions: true } },
        model,
        skills: ["engineering-workflow"],
        permissions: { read: "allow", write: "deny", delegate: "allow", review: "allow" }
      }
    },
    routing: [],
    recovery: {},
...
describe("Paseo Harness start", () => {
  it("creates a bootstrapped persistent lead and reuses it on the next start", async () => {
    ...
    const state = JSON.parse(await fs.readFile(path.join(root, ".harness/paseo/lead-session.json"), "utf8")) as { agentId: string };
    expect(state.agentId).toBe("agent-123");
    const bootstrap = await fs.readFile(path.join(root, ".harness/paseo/lead-bootstrap.md"), "utf8");
    expect(bootstrap).toContain("deterministic QUICK/SPEC triage");
  ...
  it("auto-runs toolchain setup when Paseo or the lead runtime is missing", async () => {
    ...
    const deps = {
      run: vi.fn(async (command: string) => {
        ...
        if (command.startsWith("paseo run --background --quiet")) return processResult(0, "agent-setup\n");
        if (command === "paseo wait 'agent-setup' --timeout 300") return processResult(0, "idle");
        throw new Error(`unexpected command: ${command}`);
      }) as never,
      commandExists: vi.fn(async () => reconciled) as never,
      setupToolchain: setup as never,
      loadTopology: vi.fn(async () => topology()) as never
    };
  ...
  it("builds a bootstrap that makes mutating prompts Harness-owned", () => {
    const bootstrap = buildPaseoLeadBootstrap("pawra", "/repo/pawra", "npm exec -- aeh");
    expect(bootstrap).toContain("every later user message");
    expect(bootstrap).toContain("engineering-workflow");
    expect(bootstrap).toContain("The user must not have to mention AEH");
    expect(bootstrap).toContain("never implement directly as a shortcut");
