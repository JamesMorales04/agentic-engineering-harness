...
export interface IssuePreparationResult {
  ...
  snapshot: GithubIssueSnapshot;
  normalizedBy: "planner" | "deterministic";
  traceability?: string;
}
...
const issuePlanSchema = z.object({
  ...
  scope: z.object({ allowed: z.array(z.string().min(1)).default([]), forbidden: z.array(z.string().min(1)).default([]), domains: z.array(z.string().min(1)).default([]) }),
  risk: z.enum(["low", "medium", "high"]),
  flags: z.array(flagSchema).default([]),
  constraints: z.object({ breakingApiChanges: z.boolean(), newDependencies: z.boolean(), schemaChanges: z.boolean() }),
  design: z.object({ currentState: z.string().min(1), proposedDesign: z.string().min(1), risks: z.array(z.string()).default([]) }),
  tasks: z.array(z.object({ title: z.string().min(1), requirementIndexes: z.array(z.number().int().positive()).min(1), scope: z.array(z.string()).default([]) })).min(1),
...
export async function inspectGithubIssue(root: string, config: HarnessProjectConfig, issueNumber: number): Promise<IssueInspection> {
  ...
  const acceptance = extractAcceptance(snapshot.body);
  const evidence = deriveTriageEvidence(snapshot);
...
export async function prepareGithubIssueTask(root: string, config: HarnessProjectConfig, issueNumber: number, options: { refresh?: boolean; force?: boolean; usePlanner?: boolean } = {}): Promise<IssuePreparationResult> {
  const inspection = await inspectGithubIssue(root, config, issueNumber);
  const snapshot = inspection.snapshot;
  const taskId = taskIdForIssue(issueNumber);
  const existing = await tryLoadTask(root, config, taskId);
  if (existing?.issue?.provider === "github") {
    ...
    if (options.refresh) {
      const delivery = await loadDeliveryRecord(root, config, taskId);
      if (delivery?.paseo?.workspaceId && !options.force) throw new Error("ISSUE_REFRESH_BLOCKED: an implementation workspace already exists. Review/close that workspace or use --force explicitly.");
    }
  }

  const plannerNeeded = inspection.preliminaryMode === "spec" || inspection.acceptance.length === 0;
  const normalized = plannerNeeded && options.usePlanner !== false
    ? await normalizeIssueWithPlanner(root, config, snapshot).catch(() => ({ plan: normalizeIssueDeterministically(snapshot), normalizedBy: "deterministic" as const }))
    : { plan: normalizeIssueDeterministically(snapshot), normalizedBy: "deterministic" as const };
  ...
  let traceability: string | undefined;
  ...
  if (finalDecision.quickEligible && plan.acceptance.length > 0) {
    const quick = await createQuickContract(root, config, taskId, {
      ...
      scope: plan.scope.allowed,
      acceptance: plan.acceptance.map((item) => `${item.title}: ${item.then}`),
      domains: plan.scope.domains,
      risk: "low",
      flags: []
    });
    contract = {
      ...quick.contract,
      source: { ...(quick.contract.source ?? {}), issue: snapshotPath },
      issue: issueMetadata(snapshot, snapshotPath),
      git: { ...quick.contract.git, baseRef: originatingBranch, originatingBranch },
      scope: { ...quick.contract.scope, forbidden: plan.scope.forbidden }
    };
    await writeContract(root, config, contract);
    const quickValidation = validateQuickTaskContract(config, contract);
    if (!quickValidation.ok) throw new Error(`Issue-derived QuickContract failed validation: ${quickValidation.issues.join("; ")}`);
  } else {
    contract = await writeIssueDerivedSdd(root, config, snapshot, snapshotPath, plan, originatingBranch);
    const validation = await validateSddChange(root, taskId, config);
    if (!validation.ok) throw new Error(`Issue-derived SDD failed validation: ${[...validation.missing, ...validation.issues].join("; ")}`);
    traceability = formatTraceabilityMatrix(validation.requirements);
  }

  await sealTask(root, config, contract);
  await seedDeliveryRecordFromIssue(root, config, contract, { repository: snapshot.repository, issueNumber: snapshot.number, issueUrl: snapshot.url });
...
export async function verifyGithubIssueDrift(root: string, config: HarnessProjectConfig, contract: TaskContract): Promise<{ ok: boolean; message: string; remote?: GithubIssueSnapshot }> {
  ...
  let remote: GithubIssueSnapshot;
  try { remote = await fetchGithubIssueSnapshot(root, config, contract.issue.number, contract.issue.repository); }
  catch (error) { throw new Error(`BLOCKED_EXTERNAL: cannot verify GitHub issue #${contract.issue.number} before execution: ${String(error)}`); }
...
export function normalizeIssueDeterministically(snapshot: GithubIssueSnapshot): IssueIntakePlan {
  const acceptance = extractAcceptance(snapshot.body);
  const bullets = extractUsefulBullets(snapshot.body);
  const requirementTexts = unique((acceptance.length ? acceptance : bullets).filter(Boolean)).slice(0, 20);
  const fallbackRequirement = snapshot.body.trim() ? `${snapshot.title}: ${firstMeaningfulParagraph(snapshot.body)}` : snapshot.title;
  const requirements = (requirementTexts.length ? requirementTexts : [fallbackRequirement]).map((text) => ({ text, source: "explicit" as const, validators: ["gherkin"] }));
  const paths = extractFilePaths(snapshot.body);
  const domains = inferDomains(snapshot.labels, snapshot.body);
  const request = `${snapshot.title}\n${snapshot.body}`;
  const preliminary = triageChange({ version: 1, project: { name: "issue-normalizer" } }, { request, files: paths, domains, risk: inferRisk(snapshot.labels), flags: [] });
  const risk = preliminary.quickEligible ? "low" as const : inferRisk(snapshot.labels) === "low" ? "medium" as const : inferRisk(snapshot.labels);
  const scenarios = requirements.map((item, index) => ({ title: `Satisfy requirement ${index + 1}`, requirementIndexes: [index + 1], given: `the repository is evaluated against frozen GitHub issue #${snapshot.number}`, when: "the issue implementation is completed", then: item.text }));
  const constraints = inferConstraints(request);
  return {
    classification: snapshot.body.trim() || snapshot.title.trim() ? "ready" : "requires_product_decision",
    rationale: "Requirements were normalized from the explicit issue text; repository-specific planning may refine implementation details without changing the frozen issue intent.",
    problem: firstMeaningfulParagraph(snapshot.body) || snapshot.title,
    desiredOutcome: snapshot.title,
    requirements,
    acceptance: scenarios,
    scope: { allowed: paths.length ? paths : ["**"], forbidden: [], domains },
    risk,
    flags: inferFlags(request),
    constraints,
    design: { currentState: "Use the existing repository architecture and nearby implementation patterns as the baseline.", proposedDesign: "Implement the frozen issue requirements with the smallest architecture-consistent change; routed planning may choose concrete files and tests.", risks: [] },
    tasks: requirements.map((_, index) => ({ title: `Implement requirement ${index + 1}`, requirementIndexes: [index + 1], scope: paths })),
    nonGoals: [], unresolved: []
  };
}
...
async function normalizeIssueWithPlanner(root: string, config: HarnessProjectConfig, snapshot: GithubIssueSnapshot): Promise<{ plan: IssueIntakePlan; normalizedBy: "planner" }> {
  if (!config.agents) throw new Error("No agent topology configured for issue normalization.");
  const topology = await loadResolvedAgentTopology(root, config, config.agents.activeProfile);
  const plannerName = config.workflow?.issueIntake?.plannerAgent ?? Object.values(topology.agents).find((agent) => agent.role === "planner" && !agent.disabled)?.name ?? "planner";
  if (!topology.agents[plannerName]) throw new Error(`Issue intake planner '${plannerName}' is not available.`);
  const selection = executionSelectionForAgent(topology, plannerName);
  const provisional: TaskContract = { version: 1, task: { id: taskIdForIssue(snapshot.number), title: snapshot.title }, git: { baseRef: config.validation?.baseRef ?? "main" }, scope: { allowed: ["**"], forbidden: [], frozen: [] }, routing: { intent: "plan", domains: [], risk: "medium" }, constraints: { breakingApiChanges: false, newDependencies: false, schemaChanges: false } };
  ...
  const session = await executeAgentPrompt(root, config, provisional, selection, prompt);
  if (session.exitCode !== 0) throw new Error(`Issue planner exited with ${session.exitCode}: ${session.stderr || session.stdout}`);
  return { plan: issuePlanSchema.parse(extractMarkedJson(session.stdout, session.stderr)), normalizedBy: "planner" };
}
...
async function fetchGithubIssueSnapshot(root: string, config: HarnessProjectConfig, issueNumber: number, repositoryOverride?: string): Promise<GithubIssueSnapshot> {
  ...
  const repository = repositoryOverride ?? github?.repository ?? await inferGithubRepository(root);
  const apiBase = (github?.apiBaseUrl ?? "https://api.github.com").replace(/\/$/, "");
  const token = resolveGithubTokenOptional(github?.tokenEnv);
  const issue = await githubRequest<GithubIssueResponse>(apiBase, token, `/repos/${repository}/issues/${issueNumber}`);
  if (issue.pull_request) throw new Error(`GitHub #${issueNumber} is a pull request, not an issue.`);
...
async function writeIssueDerivedSdd(root: string, config: HarnessProjectConfig, snapshot: GithubIssueSnapshot, snapshotPath: string, plan: IssueIntakePlan, originatingBranch: string): Promise<TaskContract> {
  const taskId = taskIdForIssue(snapshot.number);
  const specsDir = config.sdd?.specsDir ?? "specs";
  const dir = path.join(root, specsDir, "changes", taskId);
  await fs.mkdir(dir, { recursive: true });
  const requirementIds = plan.requirements.map((_, index) => `${taskId}-R${index + 1}`);
  const requirementLines = plan.requirements.map((item, index) => `- ${requirementIds[index]} — ${item.text} _(${item.source})_`).join("\n");
  ...
  const files: Record<string, string> = { "proposal.md": proposal, "spec.md": spec, "design.md": design, "tasks.yaml": YAML.stringify(tasks), "acceptance.feature": acceptance };
  for (const [name, content] of Object.entries(files)) await fs.writeFile(path.join(dir, name), content);
  const rel = (name: string) => path.relative(root, path.join(dir, name)).replaceAll("\\", "/");
  const contract: TaskContract = {
    version: 1, mode: "spec", task: { id: taskId, title: snapshot.title },
    source: { proposal: rel("proposal.md"), spec: rel("spec.md"), design: rel("design.md"), tasks: rel("tasks.yaml"), acceptance: rel("acceptance.feature"), issue: snapshotPath },
    issue: issueMetadata(snapshot, snapshotPath),
    git: { baseRef: originatingBranch, originatingBranch },
    scope: { allowed: plan.scope.allowed.length ? plan.scope.allowed : ["**"], forbidden: plan.scope.forbidden, frozen: [] },
    routing: { intent: "implement", domains: plan.scope.domains, risk: plan.risk },
    requirements: plan.requirements.map((item, index) => ({ id: requirementIds[index], description: item.text, validators: item.validators?.length ? item.validators : ["gherkin"] })),
