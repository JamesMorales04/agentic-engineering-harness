...
export function resolveOperationStateRoot(root: string): string { const configured = process.env.AEH_OPERATION_ID?.trim() ? process.env.AEH_CONTROL_ROOT?.trim() : undefined; return path.resolve(configured || root); }
...
export async function loadOperation(root: string, operationId: string): Promise<OperationRecordV2> { return normalizeOperationRecord(JSON.parse(await fs.readFile(operationFile(root, operationId), "utf8")) as OperationRecord); }
export async function saveOperation(root: string, record: OperationRecord): Promise<void> { const normalized = normalizeOperationRecord(record); const stateRoot = resolveOperationStateRoot(root); const file = operationFile(stateRoot, normalized.id); await fs.mkdir(path.dirname(file), { recursive: true }); await withOperationLock(file, async () => { await writeRecord(file, normalized); await appendOperationEvent(stateRoot, normalized, "operation.created", ["status", "phase"]); }); }
...
export async function acknowledgeOperationLead(root: string, operationId: string, revision: number, reason?: string): Promise<OperationRecordV2> { const current = await loadOperation(root, operationId); const now = new Date().toISOString(); return patchOperationMetadata(root, operationId, { lead: current.lead ? { ...current.lead, acknowledgedRevision: Math.max(current.lead.acknowledgedRevision, revision), acknowledgedAt: now } : undefined, notification: { ...current.notification, lastLeadWakeRevision: Math.max(current.notification.lastLeadWakeRevision, revision), lastLeadWakeAt: now, lastLeadWakeReason: reason ?? current.notification.lastLeadWakeReason } }); }
export async function markTerminalDelivered(root: string, operationId: string, attempts: number, error?: string): Promise<OperationRecordV2> { const current = await loadOperation(root, operationId); return patchOperationMetadata(root, operationId, { notification: { ...current.notification, lastLeadWakeRevision: error ? current.notification.lastLeadWakeRevision : current.revision, lastLeadWakeAt: error ? current.notification.lastLeadWakeAt : new Date().toISOString(), lastLeadWakeReason: error ? current.notification.lastLeadWakeReason : "terminal", terminalDelivered: !error, attempts, lastError: error } }); }
...
export async function updateOperationParticipant(root: string, operationId: string, agentId: string, patch: Partial<Omit<OperationParticipantRecord, "id" | "registeredAt">>): Promise<OperationRecordV2> { return mutateOperation(root, operationId, {}, true, "operation.participant.updated", (current, revision, now) => { const previous = current.participants[agentId] ?? { id: agentId, status: "REGISTERED" as const, registeredAt: now }; const next: OperationParticipantRecord = { ...previous, ...patch, id: agentId, registeredAt: previous.registeredAt, startedAt: patch.status === "RUNNING" ? previous.startedAt ?? now : patch.startedAt ?? previous.startedAt, finishedAt: ["COMPLETED", "FAILED", "CANCELLED"].includes(patch.status ?? "") ? patch.finishedAt ?? now : patch.finishedAt ?? previous.finishedAt }; const participants = { ...current.participants, [agentId]: next }; return { ...current, revision, updatedAt: now, lastProgressAt: now, participants, progress: deriveProgress(participants) }; }); }
