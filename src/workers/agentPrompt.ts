...
async function executeDirect(root: string, config: HarnessProjectConfig, selection: AgentExecutionSelection, prompt: string, options: AgentPromptOptions): Promise<WorkerSession> {
  if (selection.runtimeAdapter === "opencode") {
    ...
    return { ...session(selection, result.exitCode, result.stdout, result.stderr), id: options.resumeSessionId ?? extractSessionId(result.stdout) };
  }
...
async function executeCodex(root: string, config: HarnessProjectConfig, selection: AgentExecutionSelection, prompt: string, options: AgentPromptOptions): Promise<WorkerSession> {
  ...
  try {
    const schemaFile = temp ? path.join(temp, "schema.json") : undefined; const outputFile = temp ? path.join(temp, "output.json") : undefined;
    ...
    let stdout = result.stdout; if (outputFile) { try { stdout = await fs.readFile(outputFile, "utf8"); } catch { /* keep event stream for diagnostics */ } }
...
async function executePodman(root: string, config: HarnessProjectConfig, contract: TaskContract, selection: AgentExecutionSelection, prompt: string, options: AgentPromptOptions): Promise<WorkerSession> {
  ...
  const runtimeConfig = JSON.stringify(buildOpenCodeRuntimeConfig(selection, config)); args.push("-e", `OPENCODE_CONFIG_CONTENT=${runtimeConfig}`);
  ...
  const runtimeArgs = ["opencode", "run", "--auto", "--format", "json", "--model", selection.modelId]; if (selection.variant) runtimeArgs.push("--variant", selection.variant); if (selection.nativeAgent) runtimeArgs.push("--agent", selection.nativeAgent); runtimeArgs.push(...selection.args, prompt);
  args.push(`cd /workspace && ${runtimeArgs.map(quote).join(" ")}`);
  const result = await runProcess(args.map(quote).join(" "), { cwd: root, timeoutMs: (config.orchestration?.worker?.timeoutSeconds ?? 1800) * 1000 }); return { ...session(selection, result.exitCode, result.stdout, result.stderr), id: extractSessionId(result.stdout) };
}
