...
function legacySelection(model?: string): AgentExecutionSelection { return { logicalAgent: "legacy-worker", role: "implementer", profile: "legacy", domains: ["*"], runtimeName: "opencode", runtimeAdapter: "opencode", paseoProvider: "opencode", modelAlias: "legacy", modelProvider: model?.split("/")[0] ?? "opencode", modelName: model?.split("/").at(-1) ?? "default", modelId: model ?? "opencode/default", transport: "podman", permissions: { read: "allow", write: "allow", shell: "allow", network: "ask", delegate: "deny", review: "deny", validate: "allow", gitWrite: "deny" }, skills: [], mcps: [], args: [], runtimeCapabilities: { modelSelection: true, sessions: false, structuredOutput: false } }; }
