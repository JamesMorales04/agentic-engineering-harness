...
import { formatTriageDecision, triageChangeWithSemanticAssessment, type TriageFlag } from "./core/triage.js";
...
import { generateProvenance, verifyProvenanceManifest, verifySupplyChainGate } from "./provenance/generate.js";
...
import type { CertificationCapability } from "./certification/types.js";
...
program.command("triage").description("Assess the canonical implementation route and assurance").argument("<request>").option("--file <files...>").option("--domain <domains...>").option("--risk <risk>", "low, medium or high", "low").option("--flag <flags...>").argument("[directory]", "Project directory", ".").action(async (request: string, directory: string, options: { file?: string[]; domain?: string[]; risk?: "low" | "medium" | "high"; flag?: TriageFlag[] }) => {
  const root = path.resolve(directory); const config = await loadProjectConfig(root); const runtime = await createSemanticAssessmentRuntimeV1(root, config); const binding = await createSemanticRepositoryBindingV1(root, config); const decision = await triageChangeWithSemanticAssessment(config, { request, files: options.file, domains: options.domain, risk: options.risk, flags: options.flag }, { service: runtime.service, binding, policyRevision: runtime.policyRevision });
  console.log(formatTriageDecision(decision)); console.log(JSON.stringify(decision, null, 2));
});
...
agents.command("route").requiredOption("--intent <intent>").option("--domain <domains...>").option("--file <files...>").option("--risk <risk>").option("--profile <profile>").argument("[directory]", "Project directory", ".").action(async (directory: string, options: { intent: string; domain?: string[]; file?: string[]; risk?: "low" | "medium" | "high"; profile?: string }) => {
  const root = path.resolve(directory); const config = await loadProjectConfig(root); const topology = await loadResolvedAgentTopology(root, config, options.profile); const runtime = await createSemanticAssessmentRuntimeV1(root, config, { profile: options.profile }); const binding = await createSemanticRepositoryBindingV1(root, config);
...
certification.command("self-dogfood").argument("<fixture>", "Disposable black-box fixture directory").requiredOption("--check <executable>", "Deterministic fixture oracle executable").option("--check-args <json>", "JSON array of oracle argv arguments", "[]").option("--prompt <text>", "Optional external actor instruction").option("--capability <capability>", "Capability lane from the certification matrix").option("--require-model-e2e", "Require a proven model-backed lane for acceptance").option("--model <model>", "Bootstrap provider model", "gpt-6-luna").option("--reasoning-effort <effort>", "Bootstrap provider reasoning effort", "high").argument("[directory]", "AEH source checkout", ".").action(async (fixture: string, directory: string, options: { check: string; checkArgs: string; prompt?: string; capability?: CertificationCapability; requireModelE2E?: boolean; model: string; reasoningEffort: "low" | "medium" | "high" | "xhigh" }) => {
  const root = path.resolve(directory); const config = await loadProjectConfig(root); let args: string[];
  try { const parsed: unknown = JSON.parse(options.checkArgs); if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) throw new Error("must be a JSON string array"); args = parsed; } catch (error) { throw new Error(`--check-args must be a JSON string array: ${String(error)}`); }
  const configured = config.certification; const policy = defaultCertificationPolicy({ budget: { maxAttempts: configured?.maxAttempts ?? 1, maxDurationMs: configured?.maxDurationMs ?? 30 * 60_000, ...(configured?.maxCostUsd === undefined ? {} : { maxCostUsd: configured.maxCostUsd }), ...(configured?.maxTotalTokens === undefined ? {} : { maxTotalTokens: configured.maxTotalTokens }) } });
  ...
  console.log(JSON.stringify(report, null, 2)); if (!report.accepted) process.exitCode = 1;
});
...
provenance.command("verify").option("--manifest <file>").option("--task <id>").argument("[directory]", "Project directory", ".").action(async (directory: string, options: { manifest?: string; task?: string }) => { const root = path.resolve(directory); if (options.manifest || options.task) { const manifest = options.manifest ?? await findProvenanceManifest(root, options.task!); if (!manifest) throw new Error("provenance verify could not find a manifest for the requested task."); const result = await verifyProvenanceManifest(root, manifest); if (result.ok) console.log(`PASS provenance manifest=${manifest}`); else { for (const failure of result.failures) console.error(`FAIL ${failure}`); process.exitCode = 1; } return; } const config = await loadProjectConfig(root); const result = await verifySupplyChainGate(root, config); if (result.ok) console.log("PASS strict supply-chain gate"); else { result.failures.forEach((failure) => console.error(`FAIL ${failure}`)); process.exitCode = 1; } });
