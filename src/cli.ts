...
program.name("engineering-harness").description("Deterministic control layer for spec-driven, issue-driven and bounded quick multi-agent software engineering").version("0.4.15");
...
function printRunResult(result: TaskRunResult, mode: string): void {
  printChecks(result.report.checks);
  const humanRequired = result.review?.humanRequired ?? result.delivery?.humanRequired ?? false;
  console.log(`\n${result.status} — mode=${mode}, agent=${result.routing?.agent ?? result.worker.provider}, runtime=${result.routing?.runtime ?? result.worker.provider}, model=${result.routing?.model ?? result.worker.model ?? "default"}, profile=${result.routing?.profile ?? "legacy"}, repairs=${result.metrics.repairCount}, review=${result.review?.status ?? "skipped"}, finalState=${result.review?.finalState ?? "n/a"}, qualityRounds=${result.review?.rounds ?? 0}, debtScore=${result.review?.debtScore ?? "n/a"}, convergence=${result.review?.convergence ?? "n/a"}, delivery=${result.delivery?.status ?? "skipped"}, pr=${result.delivery?.pullRequest?.url ?? "n/a"}, humanRequired=${humanRequired}, firstPass=${result.metrics.firstPassSuccess}, tokens=${result.metrics.usage.totalTokens ?? "n/a"}, costUsd=${result.metrics.usage.costUsd ?? "n/a"}`);
