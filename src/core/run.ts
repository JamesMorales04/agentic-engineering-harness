...
export async function runTask(root: string, config: HarnessProjectConfig, contract: TaskContract, options?: { profile?: string }): Promise<TaskRunResult> {
  ...
  if (effectiveContract.mode === "quick") {
    const quick = validateQuickTaskContract(effectiveConfig, effectiveContract); if (!quick.ok) throw new Error(`QuickContract validation failed before delegation:\n${quick.issues.map((item) => `- ${item}`).join("\n")}\nEscalate this change to SDD/spec mode.`);
  } else {
    const trace = await validateSddChange(workspaceRoot, effectiveContract.task.id, effectiveConfig); if (!trace.ok) throw new Error(`SDD validation failed before delegation:\n${[...trace.missing, ...trace.issues].map((item) => `- ${item}`).join("\n")}`);
  }
  if (workspaceRoot === controlRoot) await sealTask(controlRoot, effectiveConfig, effectiveContract);
  else { const seal = await verifyTaskSeal(workspaceRoot, effectiveContract, effectiveConfig.validation?.requireSeal ?? true); if (seal.status === "FAIL") throw new Error(`Delivery workspace trust check failed before delegation: ${seal.message}`); }

  const topologyState = await resolveTopology(controlRoot, effectiveConfig, effectiveContract, options?.profile); const topology = topologyState.topology; const route = topologyState.route; let selection = topologyState.selection;
  ...
  catch (error) { if (effectiveConfig.controlPlane?.required !== false) throw error; await recordEvent(controlRoot, effectiveConfig, "harness.control.snapshot-failed", { taskId: effectiveContract.task.id, error: String(error) }); }
  ...
  await refreshGraphIfConfigured(workspaceRoot, effectiveConfig); const beforeSnapshot = await snapshotGraph(workspaceRoot, effectiveConfig, effectiveContract.task.id, "before"); if (!beforeSnapshot && effectiveConfig.codeIntelligence?.required) throw new Error("Code intelligence is required but the Graphify before snapshot could not be created.");

  let worker: WorkerSession;
  let waveResult: PlannerWaveResult | undefined;
  let executionSessions: WorkerSession[] = [];
  let report: ValidationReport;
  if (topology && route && selection && effectiveConfig.workflow?.planning?.enabled !== false && effectiveContract.mode !== "quick") {
  ...
  if (waveResult?.used && waveResult.aggregateSession) {
    ...
    const executor = createWorkerExecutor(effectiveConfig, selection); const health = await executor.doctor(workspaceRoot, effectiveConfig, selection); if (!health.ok) throw new Error(`${executor.name} executor unavailable: ${health.message}`); worker = await executor.start(workspaceRoot, effectiveConfig, effectiveContract, selection); executionSessions.push(worker); report = withWorkerExecutionCheck(await verifyAfterWorker(workspaceRoot, controlRoot, effectiveConfig, effectiveContract, controller), worker);
  }
  ...
  while (report.status === "FAIL" && attempts < maxRepairs) {
    attempts += 1; const failureType = classifyFailure({ report, worker }); const recovery = topology ? resolveRecoveryStep(topology, failureType, attempts) : { action: "same-agent" as const }; const recoveryAction = formatRecoveryAction(recovery, selection?.logicalAgent ?? "legacy-worker"); const packet = createRepairPacket(report, attempts, { failureType, failedAgent: selection?.logicalAgent, recoveryAction }); if (!packet.failures.length) break; await writeRepairPacket(controlRoot, effectiveConfig, packet); await recordEvent(controlRoot, effectiveConfig, "harness.repair.start", { taskId: effectiveContract.task.id, attempt: attempts, failureType, recoveryAction, failures: packet.failures.length });
    if (recovery.action === "lead" || recovery.action === "stop") break;
    if (topology && recovery.action === "agent" && recovery.agent) selection = executionSelectionForAgent(topology, recovery.agent); else if (topology && recovery.action === "reroute") { const fallback = selectFallbackExecution(topology, effectiveContract, selection?.logicalAgent ?? ""); if (!fallback) break; selection = fallback; }
    if (selection) { selection = enforceSandboxPolicy(selection, effectiveConfig, effectiveContract.routing?.risk ?? "low").selection; const transport = selection.transport === "inherit" ? (effectiveConfig.orchestration?.provider ?? "none") : selection.transport; const issues = validateExecutionCapabilities(selection, transport); if (issues.length) throw new Error(`Recovery agent ${selection.logicalAgent} is not executable: ${issues.join("; ")}`); }
    executor = createWorkerExecutor(effectiveConfig, selection); const recoveryHealth = await executor.doctor(workspaceRoot, effectiveConfig, selection); if (!recoveryHealth.ok) throw new Error(`${executor.name} recovery executor unavailable: ${recoveryHealth.message}`); worker = await executor.repair(workspaceRoot, effectiveConfig, effectiveContract, worker, packet, selection); executionSessions.push(worker); report = withWorkerExecutionCheck(await verifyAfterWorker(workspaceRoot, controlRoot, effectiveConfig, effectiveContract, controller), worker); report = await attachEvidence(report); await recordEvent(controlRoot, effectiveConfig, "harness.repair.finish", { taskId: effectiveContract.task.id, attempt: attempts, status: report.status, agent: selection?.logicalAgent });
  }

  let reviewSummary: TaskRunResult["review"]; let reviewFindings: import("../agents/outputContracts.js").NormalizedFinding[] = []; let reviewSessions: WorkerSession[] = [];
  if (report.status === "PASS" && topology && route && selection) {
    ...
    report = mergeChecks(withWorkerExecutionCheck(review.report, worker), review.checks); reviewFindings = review.findings.findings; reviewSessions = review.sessions;
    const quality = review.qualityHistory.at(-1)!; reviewSummary = { status: review.status, finalState: review.finalState, humanRequired: review.humanRequired, rounds: review.rounds, findings: review.findings.outputCount, debtScore: quality.debtScore, debtPoints: quality.debtPoints, counts: quality.counts, convergence: quality.convergence, leadAccepted: review.leadAccepted, reviewerSessions: review.sessions.length };
    ...
    if (report.status === "PASS" && effectiveConfig.evidence?.enabled === true) { evidenceGraph = await buildRequirementEvidenceGraph({ root: workspaceRoot, stateRoot: controlRoot, config: effectiveConfig, contract: effectiveContract, report, plan: waveResult?.plan, findings: reviewFindings, sessions: [...executionSessions, ...reviewSessions] }); report = mergeChecks(report, [evidenceValidationCheck(evidenceGraph, effectiveConfig)]); }
  }

  let deliverySummary: DeliveryFinalizationResult | undefined;
  if (report.status === "PASS") {
    try { deliverySummary = await finalizeAcceptedIssue(workspaceRoot, effectiveConfig, effectiveContract); if (deliverySummary.status !== "SKIPPED") await recordEvent(controlRoot, effectiveConfig, "harness.delivery.finalize", { taskId: effectiveContract.task.id, status: deliverySummary.status, commitSha: deliverySummary.commitSha, pullRequest: deliverySummary.pullRequest }); }
    catch (error) { deliverySummary = deliveryFinalizationFailure(error); report = mergeChecks(report, [{ id: "delivery.finalization", category: "delivery", status: "FAIL", message: deliverySummary.message, details: { status: deliverySummary.status, humanRequired: deliverySummary.humanRequired } }]); if (reviewSummary && deliverySummary.humanRequired) reviewSummary = { ...reviewSummary, status: "FAIL", finalState: "BLOCKED_EXTERNAL", humanRequired: true }; await recordEvent(controlRoot, effectiveConfig, "harness.delivery.finalize", { taskId: effectiveContract.task.id, status: deliverySummary.status, humanRequired: deliverySummary.humanRequired, message: deliverySummary.message }); }
  }
  if (report.status === "PASS" && effectiveConfig.evidence?.enabled === true) { evidenceGraph = await buildRequirementEvidenceGraph({ root: workspaceRoot, stateRoot: controlRoot, config: effectiveConfig, contract: effectiveContract, report, plan: waveResult?.plan, findings: reviewFindings, sessions: [...executionSessions, ...reviewSessions], delivery: deliverySummary }); report = mergeChecks(report, [evidenceValidationCheck(evidenceGraph, effectiveConfig)]); }
  ...
  const usageText = [...executionSessions, ...reviewSessions].map((session) => `${session.stdout}\n${session.stderr}`).join("\n"); worker.metrics = extractUsageMetrics(usageText || `${worker.stdout}\n${worker.stderr}`); const metrics = buildRunMetrics({ firstPassSuccess, repairCount: attempts, humanInterventions: await countHumanInterventions(controlRoot, effectiveConfig, effectiveContract.task.id, startedAt), durationMs: Date.now() - startedMs, usage: worker.metrics }); const routing = selection ? { profile: selection.profile, ruleIds: route?.ruleIds ?? [], agent: selection.logicalAgent, runtime: selection.runtimeName, model: selection.modelId, nativeAgent: selection.nativeAgent, reviewers: route?.reviewers ?? [], validators: route?.validators ?? [] } : undefined;
...
async function verifyAfterWorker(workspaceRoot: string, controlRoot: string, config: HarnessProjectConfig, contract: TaskContract, controller?: ControlPlaneSnapshot): Promise<ValidationReport> { await refreshGraphIfConfigured(workspaceRoot, config); const afterSnapshot = await snapshotGraph(workspaceRoot, config, contract.task.id, "after"); if (!afterSnapshot && config.codeIntelligence?.required) throw new Error("Code intelligence is required but the Graphify after snapshot could not be created."); return verifyTask(workspaceRoot, config, contract, { stateRoot: controlRoot, policyRoot: controller?.materializedRoot ?? controlRoot }); }
