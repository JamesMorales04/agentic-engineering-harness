...
const DEFAULT_CONTROL_ROOTS = [
  ".harness/project.yaml",
  ".harness/agents.source.jsonc",
  ".harness/generated/agents.json",
...
export async function createControlPlaneSnapshot(root: string, config: HarnessProjectConfig, taskId: string): Promise<ControlPlaneSnapshot> {
  ...
  const snapshot: ControlPlaneSnapshot = {
    ...
    aehVersion: await readPackageVersion(sourceRoot),
    gitCommit: await readGitCommit(sourceRoot),
    sourceRoot,
    materializedRoot,
    includeRoots,
    files,
    compositeSha256: compositeHash(files)
  };
...
export async function materializeControlPlaneSnapshot(snapshot: ControlPlaneSnapshot, targetRoot: string, config: HarnessProjectConfig): Promise<ControlPlaneSnapshot> {
  ...
  const materialized: ControlPlaneSnapshot = { ...snapshot, materializedRoot: destinationFiles };
...
export async function detectControlPlaneDrift(root: string, snapshot: ControlPlaneSnapshot): Promise<ControlPlaneDrift> {
  ...
  for (const [relative, file] of expected) {
    ...
    if (sha256(content) !== file.sha256) changed.push(relative);
  }
  for (const relative of currentFiles) if (!expected.has(relative)) added.push(relative);
  changed.sort(); missing.sort(); added.sort();
  return { changed, missing, added, drifted: changed.length + missing.length + added.length > 0 };
}

export function controlPlanePolicyRoot(snapshot: ControlPlaneSnapshot): string {
  return snapshot.materializedRoot;
}

export async function loadControlPlaneSnapshot(root: string, config: HarnessProjectConfig, taskId: string): Promise<ControlPlaneSnapshot | undefined> {
  const file = path.resolve(root, config.controlPlane?.snapshotDir ?? ".harness/controller", taskId, "manifest.json");
  try { return JSON.parse(await fs.readFile(file, "utf8")) as ControlPlaneSnapshot; } catch { return undefined; }
}

async function resolveControlRoots(root: string, config: HarnessProjectConfig): Promise<string[]> {
  const configured = [
    config.agents?.configPath,
    config.agents?.generatedPath,
    config.toolchain?.configPath,
    config.toolchain?.lockPath,
    ...(config.validation?.opa?.policyDirs ?? []),
    ...(config.controlPlane?.include ?? [])
  ].filter((value): value is string => Boolean(value));
  const self = await isHarnessRepository(root) ? SELF_CONTROLLER_ROOTS : [];
  return [...new Set([...DEFAULT_CONTROL_ROOTS, ...configured, ...self].map(normalizeRelative))].sort();
}

async function isHarnessRepository(root: string): Promise<boolean> {
  try {
    const value = JSON.parse(await fs.readFile(path.join(root, "package.json"), "utf8")) as { name?: string };
    return value.name === "agentic-engineering-harness";
  } catch { return false; }
}

async function enumerateControlFiles(root: string, includeRoots: string[]): Promise<string[]> {
  const result = new Set<string>();
  for (const relative of includeRoots) await collectPath(root, relative, result);
  return [...result].sort();
}

async function collectPath(root: string, relative: string, result: Set<string>): Promise<void> {
  const absolute = path.resolve(root, relative);
  if (!inside(root, absolute)) throw new Error(`Control-plane snapshot path escapes project root: ${relative}`);
  let stat;
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    if (["node_modules", ".git", "dist", ".harness/controller", ".harness/runs", ".harness/reports"].includes(entry.name)) continue;
    await collectPath(root, normalizeRelative(path.relative(root, path.join(absolute, entry.name))), result);
  }
}

function compositeHash(files: ControlPlaneFile[]): string {
  const hash = crypto.createHash("sha256");
  for (const file of [...files].sort((a, b) => a.path.localeCompare(b.path))) hash.update(`${file.path}\0${file.sha256}\0${file.size}\n`);
  return hash.digest("hex");
}

async function readPackageVersion(root: string): Promise<string | undefined> {
  try { return (JSON.parse(await fs.readFile(path.join(root, "package.json"), "utf8")) as { version?: string }).version; } catch { return undefined; }
}
