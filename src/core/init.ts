...
export async function initializeProject(root: string): Promise<string[]> {
  ...
  const assets = await reconcileHarnessAssets(root); created.push(...assets.created);
