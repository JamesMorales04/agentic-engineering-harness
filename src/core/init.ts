...
async function copyTree(source: string, destination: string): Promise<void> {
  ...
  for (const entry of await fs.readdir(source, { withFileTypes: true })) {
    ...
    else await copyFileIfMissing(src, dst);
  }
}
...
export async function initializeProject(root: string): Promise<string[]> {
  ...
  for (const [src, dst] of mappings) {
    ...
    if (!(await exists(target))) {
      await copyFileIfMissing(path.join(pkg, src), target);
      created.push(dst);
    }
  }
  ...
  for (const dir of [".harness/contracts", ".harness/seals", ".harness/reports", ".harness/telemetry", "specs/changes", "docs/decisions"] ) {
