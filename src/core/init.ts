...
export async function initializeProject(root: string): Promise<string[]> {
  ...
  const mappings: Array<[string, string]> = [
    ["templates/project.yaml", ".harness/project.yaml"],
    ["templates/AGENTS.md", "AGENTS.md"],
    ["templates/otel-collector.yaml", ".harness/otel-collector.yaml"]
  ];
  for (const [src, dst] of mappings) {
    const target = path.join(root, dst);
    if (!(await exists(target))) { await copyFileIfMissing(path.join(pkg, src), target); created.push(dst); }
  }
  ...
  for (const dir of [".harness/contracts", ".harness/seals", ".harness/reports", ".harness/repairs", ".harness/runs", ".harness/telemetry", ".harness/evals/results", ".harness/evals/workspaces", ".harness/provenance", "specs/changes", "docs/decisions", "evals/corpus", "memory-benchmarks"] ) await fs.mkdir(path.join(root, dir), { recursive: true });
  return created;
}
