...
export async function verifyTask(root: string, config: HarnessProjectConfig, contract: TaskContract, options: VerifyTaskOptions = {}): Promise<ValidationReport> {
  ...
  const policyRoot = options.policyRoot ?? stateRoot;
  const startedAt = new Date().toISOString(); const baseRef = contract.git?.baseRef ?? config.validation?.baseRef ?? "HEAD";
  await recordEvent(stateRoot, config, "harness.verify.start", { taskId: contract.task.id, baseRef, workspaceRoot: root === stateRoot ? undefined : root });
  ...
  checks.push(await runOpaPolicies(root, config, contract, changedFiles, frozenChanged, evidence, policyRoot));
