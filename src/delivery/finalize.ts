...
export interface DeliveryFinalizationResult {
  status: "SKIPPED" | "NO_CHANGES" | "FINALIZED";
  committed: boolean;
  commitSha?: string;
  pushed: boolean;
...
export async function finalizeAcceptedIssue(root: string, config: HarnessProjectConfig, contract: TaskContract): Promise<DeliveryFinalizationResult> {
  ...
  if (current && current !== branch) throw new Error(`SYSTEM_FAILURE: refusing to finalize ${contract.task.id}; workspace branch is '${current}' but delivery branch is '${branch}'.`);
  ...
  if (status.stdout.trim()) {
    ...
    const commit = await runProcess(`git commit -m ${quote(`${contract.task.id}: ${contract.task.title}`)}`, { cwd: root, timeoutMs: 120_000 });
  ...
  if (Number(ahead.stdout.trim() || "0") === 0) return { status: "NO_CHANGES", committed, commitSha, pushed: false, message: `No commits differ from ${base}; no pull request is required.` };
  ...
  if (!pr) {
    pr = await githubRequest<GithubPullRequest>(apiBase, token, `/repos/${repository}/pulls`, {
      method: "POST",
      body: JSON.stringify({
        title: `${contract.task.id}: ${contract.task.title}`,
        head: branch,
        base,
        draft: github.pullRequestDraft ?? true,
        body: buildPullRequestBody(contract)
      })
    });
  }
  ...
  return {
    ...
    commitSha,
    pushed: true,
    pullRequest: { number: pr.number, url: pr.html_url, draft: pr.draft ?? (github.pullRequestDraft ?? true) },
    message: `Accepted issue task finalized on ${branch}; pull request #${pr.number}.`
  };
}

function buildPullRequestBody(contract: TaskContract): string {
  const issue = contract.issue!;
  const keyword = "Closes";
...
function skipped(message: string): DeliveryFinalizationResult { return { status: "SKIPPED", committed: false, pushed: false, message }; }
