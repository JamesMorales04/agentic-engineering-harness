...
export async function handoffSdd(root: string, config: HarnessProjectConfig, taskId: string): Promise<DeliveryRecord> {
  ...
  if (github?.enabled) {
    ...
    const repository = github.repository ?? await inferGithubRepository(root);
    ...
    if (!record.github.issueNumber) {
      ...
      record.github.issueNumber = issue.number; record.github.issueUrl = issue.html_url; record.status = "issue-created"; record.updatedAt = new Date().toISOString();
      await saveDeliveryRecord(root, config, record);
    }
    ...
    if (!record.github.branch) {
      ...
      record.github.branch = branch; record.github.branchSha = baseRef.object.sha; record.status = "branch-created"; record.updatedAt = new Date().toISOString();
      await saveDeliveryRecord(root, config, record);
    }
  }

  const paseo = config.delivery?.paseo;
  if (paseo?.enabled && paseo.createWorkspace !== false && !record.paseo?.workspaceId) {
    ...
    record.paseo = workspace; record.status = "workspace-created"; record.updatedAt = new Date().toISOString();
    await saveDeliveryRecord(root, config, record);
  }
...
export async function loadDeliveryRecord(root: string, config: HarnessProjectConfig, taskId: string): Promise<DeliveryRecord | undefined> {
  try { return JSON.parse(await fs.readFile(deliveryFile(root, config, taskId), "utf8")) as DeliveryRecord; } catch { return undefined; }
}

export async function deliveryWorkspaceId(root: string, config: HarnessProjectConfig, taskId: string): Promise<string | undefined> {
  if (config.delivery?.paseo?.autoUseWorkspace === false) return undefined;
  return (await loadDeliveryRecord(root, config, taskId))?.paseo?.workspaceId;
}

export function parseGithubRepository(remote: string): string | undefined {
  const ssh = remote.match(/^[^@]+@[^:]+:([^/]+\/[^/]+?)(?:\.git)?$/); if (ssh) return ssh[1];
  try { const url = new URL(remote); const value = url.pathname.replace(/^\//, "").replace(/\.git$/, ""); return /^[^/]+\/[^/]+$/.test(value) ? value : undefined; } catch { return undefined; }
}

export function renderPattern(pattern: string, contract: TaskContract, issue?: number): string {
  const slug = slugify(contract.task.title); return pattern.replaceAll("{issue}", issue ? String(issue) : "no-issue").replaceAll("{task}", slugify(contract.task.id)).replaceAll("{slug}", slug);
}

export async function renderIssueBody(root: string, contract: TaskContract, originatingBranch: string): Promise<string> {
  const spec = contract.source?.spec ? await fs.readFile(path.resolve(root, contract.source.spec), "utf8") : "";
  const acceptance = contract.source?.acceptance ? await fs.readFile(path.resolve(root, contract.source.acceptance), "utf8") : "";
  const requirements = (contract.requirements ?? []).map((item) => `- ${item.id}: ${item.description ?? "see specification"}`).join("\n");
...
async function runPaseoWorkspace(root: string, mode: string, slug: string, title: string, branch: string): Promise<{ workspaceId?: string; worktreePath?: string }> { const create = await runProcess(`paseo workspace create --isolation worktree --path ${quote(root)} ${mode} --worktree-slug ${quote(slug)} --title ${quote(title)} --json`, { cwd: root, timeoutMs: 180_000 }); if (create.exitCode !== 0) throw new Error(`Paseo workspace creation failed: ${create.stderr || create.stdout}`); const direct = parseWorkspace(create.stdout, branch); if (direct.workspaceId) return direct; const list = await runProcess("paseo workspace ls --json", { cwd: root, timeoutMs: 60_000 }); if (list.exitCode === 0) { const found = parseWorkspace(list.stdout, branch); if (found.workspaceId) return found; } throw new Error(`Paseo created the workspace but its ID could not be resolved. Output: ${create.stdout}`); }
function parseWorkspace(raw: string, branch: string): { workspaceId?: string; worktreePath?: string } { try { const value = JSON.parse(raw) as unknown; const candidates = flattenObjects(value); const found = candidates.find((item) => [item.branch, item.branchName, item.gitBranch].some((candidate) => candidate === branch)) ?? candidates.find((item) => typeof item.id === "string" || typeof item.workspaceId === "string"); return found ? { workspaceId: stringValue(found.workspaceId) ?? stringValue(found.id), worktreePath: stringValue(found.worktreePath) ?? stringValue(found.path) } : {}; } catch { return {}; } }
