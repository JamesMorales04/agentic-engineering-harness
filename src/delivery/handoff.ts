...
export async function deliveryWorkspaceId(root: string, config: HarnessProjectConfig, taskId: string): Promise<string | undefined> { if (config.delivery?.paseo?.autoUseWorkspace === false) return undefined; return (await loadDeliveryRecord(root, config, taskId))?.paseo?.workspaceId; }
export async function deliveryWorkspacePath(root: string, config: HarnessProjectConfig, taskId: string): Promise<string | undefined> { if (config.delivery?.paseo?.autoUseWorkspace === false) return undefined; const value = (await loadDeliveryRecord(root, config, taskId))?.paseo?.worktreePath; return value && await exists(value) ? value : undefined; }
