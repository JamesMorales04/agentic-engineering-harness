...
export async function executePlannerWaves(input: { root: string; stateRoot: string; config: HarnessProjectConfig; contract: TaskContract; topology: ResolvedAgentTopology; implementationSelection: AgentExecutionSelection; controller?: ControlPlaneSnapshot; precomputedPlan?: PlannerOutput; revalidate: () => Promise<ValidationReport>; }): Promise<PlannerWaveResult> {
  ...
  await recordEvent(input.stateRoot, input.config, "harness.plan.ready", { taskId: input.contract.task.id, tasks: plan.tasks.length, waves: schedule.waves.length, conflicts: schedule.conflicts.length, graphUsed: schedule.graphUsed, distributed: planning?.distributed === true && input.config.distributed?.enabled === true });
  const acceptedPatches: string[] = []; const waveSummaries: WaveExecutionSummary[] = []; let finalReport: ValidationReport | undefined;
  for (let index = 0; index < schedule.waves.length; index += 1) {
    const ids = schedule.waves[index]; const tasks = ids.map((id) => plan.tasks.find((task) => task.id === id)!).filter(Boolean);
    const results = await mapLimit(tasks, planning?.maxWaveConcurrency ?? tasks.length, (task) => executeDelegation({ ...input, task, priorPatches: acceptedPatches })); sessions.push(...results.map((result) => result.session));
    if (results.some((result) => result.status === "FAIL")) { const summary: WaveExecutionSummary = { wave: index + 1, taskIds: ids, status: "FAIL", results }; waveSummaries.push(summary); await recordEvent(input.stateRoot, input.config, "harness.wave.finish", { taskId: input.contract.task.id, wave: index + 1, status: "FAIL", tasks: ids }); return { used: true, plan, schedule, waves: waveSummaries, sessions, aggregateSession: aggregate(sessions, 1, `Wave ${index + 1} failed.`) }; }
    for (const result of results) { if (!result.patch.trim()) continue; const check = await runProcess("git apply --check --binary -", { cwd: input.root, timeoutMs: 60_000, stdin: result.patch }); if (check.exitCode !== 0) { result.status = "FAIL"; result.message = `Patch integration check failed: ${check.stderr || check.stdout}`; const summary: WaveExecutionSummary = { wave: index + 1, taskIds: ids, status: "FAIL", results }; waveSummaries.push(summary); return { used: true, plan, schedule, waves: waveSummaries, sessions, aggregateSession: aggregate(sessions, 1, result.message) }; } }
    for (const result of results) { if (!result.patch.trim()) continue; const apply = await runProcess("git apply --binary -", { cwd: input.root, timeoutMs: 60_000, stdin: result.patch }); if (apply.exitCode !== 0) throw new Error(`Previously checked wave patch failed to apply: ${apply.stderr || apply.stdout}`); acceptedPatches.push(result.patch); }
  ...
  finalReport ??= await input.revalidate(); return { used: true, plan, schedule, waves: waveSummaries, sessions, aggregateSession: aggregate(sessions, finalReport.status === "PASS" ? 0 : 1, `Executed ${plan.tasks.length} task(s) across ${schedule.waves.length} wave(s).`), report: finalReport };
}

async function executeDelegation(input: { root: string; stateRoot: string; config: HarnessProjectConfig; contract: TaskContract; topology: ResolvedAgentTopology; implementationSelection: AgentExecutionSelection; controller?: ControlPlaneSnapshot; task: DelegationTask; priorPatches: string[]; }): Promise<DelegationExecutionResult> {
  let selection = input.topology.agents[input.task.agent] ? executionSelectionForAgent(input.topology, input.task.agent) : input.implementationSelection;
  try { selection = enforceSandboxPolicy(selection, input.config, input.task.risk).selection; } catch (error) { return failed(input.task, selection, String(error)); }
  ...
  if (input.config.workflow?.planning?.distributed === true && input.config.distributed?.enabled === true) {
    try { const remote = await dispatchDistributedDelegation({ root: input.root, config: input.config, contract: input.contract, task: input.task, selection, prompt, priorPatches: input.priorPatches, controller: input.controller }); const violations = remote.changedFiles.filter((file) => !matchesAny(file, input.task.scope)); if (violations.length) return { task: input.task, session: remote.session, changedFiles: remote.changedFiles, patch: "", status: "FAIL", distributed: true, message: `Remote delegation escaped scope: ${violations.join(", ")}` }; return { task: input.task, session: remote.session, changedFiles: remote.changedFiles, patch: remote.patch, status: remote.status, distributed: true, message: remote.message }; }
    catch (error) { return failed(input.task, selection, `Distributed delegation failed: ${String(error)}`, true); }
  }
  const worktree = await fs.mkdtemp(path.join(os.tmpdir(), `aeh-${safe(input.contract.task.id)}-${safe(input.task.id)}-`)); const add = await runProcess(`git worktree add --detach ${quote(worktree)} HEAD`, { cwd: input.root, timeoutMs: 120_000 }); if (add.exitCode !== 0) return failed(input.task, selection, `Unable to create task worktree: ${add.stderr || add.stdout}`);
  try {
    for (const patchText of input.priorPatches) { const applied = await runProcess("git apply --binary -", { cwd: worktree, timeoutMs: 60_000, stdin: patchText }); if (applied.exitCode !== 0) return failed(input.task, selection, `Unable to materialize prior wave state: ${applied.stderr || applied.stdout}`); }
    await copyTaskContext(input.root, worktree, input.config, input.contract); if (input.controller) await materializeControlPlaneSnapshot(input.controller, worktree, input.config); await runProcess("git add -A && git -c user.name=aeh -c user.email=aeh@localhost commit --no-gpg-sign -m 'aeh wave baseline' --allow-empty", { cwd: worktree, timeoutMs: 60_000 });
    const session = await executeAgentPrompt(worktree, input.config, input.contract, selection, prompt); if (session.exitCode !== 0) return { task: input.task, session, changedFiles: [], patch: "", status: "FAIL", message: `Agent exited with ${session.exitCode}.` };
    ...
    const diff = await runProcess("git diff --binary --no-ext-diff HEAD", { cwd: worktree, timeoutMs: 60_000 }); if (diff.exitCode !== 0) return { task: input.task, session, changedFiles, patch: "", status: "FAIL", message: diff.stderr || "Unable to capture delegation patch." }; return { task: input.task, session, changedFiles, patch: diff.stdout, status: "PASS" };
  } finally { await runProcess(`git worktree remove --force ${quote(worktree)}`, { cwd: input.root, timeoutMs: 120_000 }); await fs.rm(worktree, { recursive: true, force: true }).catch(() => undefined); }
}

export function validatePlannerWavePlan(contract: TaskContract, topology: ResolvedAgentTopology, plan: PlannerOutput): string[] {
  ...
  for (const task of plan.tasks) { if (ids.has(task.id)) issues.push(`duplicate task id ${task.id}`); ids.add(task.id); if (!topology.agents[task.agent]) issues.push(`${task.id} references unknown agent ${task.agent}`); if (!task.scope.length) issues.push(`${task.id} has empty scope`); for (const scope of task.scope) if (!withinContractScope(scope, contract.scope?.allowed ?? ["**"])) issues.push(`${task.id} scope ${scope} is outside TaskContract scope`); for (const req of task.acceptance) { if (requirements.size && !requirements.has(req)) issues.push(`${task.id} references unknown requirement ${req}`); if (requirements.has(req)) covered.add(req); } }
  for (const task of plan.tasks) for (const dependency of task.dependencies) if (!ids.has(dependency)) issues.push(`${task.id} depends on unknown task ${dependency}`); for (const requirement of requirements) if (!covered.has(requirement)) issues.push(`requirement ${requirement} is not assigned to any implementation task`); return [...new Set(issues)];
}
