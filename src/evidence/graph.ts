...
function normalizedRequirements(contract: TaskContract): Array<{ id: string; description: string; validators: string[]; capabilities: string[] }> { if (contract.requirements?.length) return contract.requirements.map((item) => ({ id: item.id, description: item.description ?? item.id, validators: [...new Set([...(item.validators ?? []), ...(item.validator ? [item.validator] : [])])], capabilities: [...new Set(item.capabilities ?? [])] })); const validators = [...(contract.verification?.validators ?? []).map((item) => item.id), ...(contract.verification?.commands ?? []).map((item) => item.id)]; const capabilities = [...new Set(contract.verification?.capabilities ?? [])]; const quickValidators = contract.mode === "quick" ? (contract.quick?.acceptance ?? []).map((_, index) => `quick.acceptance.${index + 1}`) : []; return (contract.quick?.acceptance ?? []).map((description, index) => ({ id: `QUICK-AC-${index + 1}`, description, validators: [...validators, ...(quickValidators[index] ? [quickValidators[index]] : [])], capabilities })); }
