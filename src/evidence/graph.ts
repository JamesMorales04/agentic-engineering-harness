...
export interface RequirementEvidenceGraph { version: 1; taskId: string; createdAt: string; candidate?: CandidateRevisionV1; candidateWorkspaceIdentity?: CandidateWorkspaceIdentityEvidenceV1; nodes: EvidenceNode[]; edges: EvidenceEdge[]; requirements: RequirementCoverage[]; complete: boolean; reasons: string[]; sha256: string; }
...
export async function buildRequirementEvidenceGraph(input: { root: string; stateRoot?: string; config: HarnessProjectConfig; contract: TaskContract; report: ValidationReport; plan?: PlannerOutput; findings?: NormalizedFinding[]; sessions?: WorkerSession[]; delivery?: DeliveryFinalizationResult; }): Promise<RequirementEvidenceGraph> {
  ...
  const currentOperation = operationId ? await loadOperation(resolveOperationStateRoot(stateRoot), operationId) : undefined;
  const candidateWorkspaceIdentity = input.report.candidate ? await assertWorkspaceMatchesCandidate(input.root, input.report.candidate, operationId ? currentOperation?.candidateRevision ?? null : undefined) : undefined;
  if (candidateWorkspaceIdentity && input.report.candidateWorkspaceIdentity && input.report.candidateWorkspaceIdentity.candidateIdentityDigest !== candidateWorkspaceIdentity.candidateIdentityDigest) throw new Error("CANDIDATE_WORKSPACE_MISMATCH: validation report identity evidence is bound to another candidate.");
  const runId = `run:${input.contract.task.id}`; const nodes = new Map<string, EvidenceNode>(); const edges: EvidenceEdge[] = [];
  addNode(nodes, { id: runId, type: "run", label: input.contract.task.title, data: { route: input.contract.routing?.route ?? "DIRECT", assurance: input.contract.routing?.assurance ?? "STANDARD", status: input.report.status, candidateDigest: input.report.candidate?.identityDigest, sourceDigest: input.report.candidate?.sourceDigest, candidateWorkspaceIdentity } });
  const requirements = normalizedRequirements(input.contract);
  const planTasks = input.plan?.workUnits?.length ? input.plan.workUnits : [{ id: input.contract.task.id, objective: input.contract.task.title, scope: input.contract.scope?.allowed ?? ["**"], dependencies: [], requirementRefs: requirements.map((item) => item.id), acceptanceRefs: [], competencies: ["general-engineering"], riskTags: [], changeKinds: ["source", "test"] as const, risk: input.contract.routing?.risk ?? "low" as const }];
  ...
  for (const task of planTasks) { const id = `task:${task.id}`; addNode(nodes, { id, type: "task", label: task.objective, data: { competencies: task.competencies, risk: task.risk, scope: task.scope, dependencies: task.dependencies } }); addEdge(edges, runId, id, "contains"); for (const req of [...task.requirementRefs, ...task.acceptanceRefs]) if (requirements.some((item) => item.id === req)) addEdge(edges, `req:${req}`, id, "implemented-by"); }
  ...
  const payload = { version: 1 as const, taskId: input.contract.task.id, createdAt: new Date().toISOString(), ...(input.report.candidate ? { candidate: input.report.candidate } : {}), ...(candidateWorkspaceIdentity ? { candidateWorkspaceIdentity } : {}), nodes: [...nodes.values()].sort((a, b) => a.id.localeCompare(b.id)), edges: uniqueEdges(edges), requirements: coverage, complete: coverage.every((item) => item.complete), reasons };
  const graph: RequirementEvidenceGraph = { ...payload, sha256: digest(payload) }; const output = path.resolve(stateRoot, input.config.evidence?.outputDir ?? ".harness/evidence", `${input.contract.task.id}.json`); await fs.mkdir(path.dirname(output), { recursive: true });
  if (input.report.candidate) {
    const latestOperation = operationId ? await loadOperation(resolveOperationStateRoot(stateRoot), operationId) : undefined;
    await assertWorkspaceMatchesCandidate(input.root, input.report.candidate, operationId ? latestOperation?.candidateRevision ?? null : undefined);
  }
...
function normalizedRequirements(contract: TaskContract): Array<{ id: string; description: string; validators: string[]; capabilities: string[] }> { return contract.requirements?.length ? contract.requirements.map((item) => ({ id: item.id, description: item.description ?? item.id, validators: [...new Set([...(item.validators ?? []), ...(item.validator ? [item.validator] : [])])], capabilities: [...new Set(item.capabilities ?? [])] })) : []; }
