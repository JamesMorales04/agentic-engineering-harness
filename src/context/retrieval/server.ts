...
import { createPersistedContextGateway, retrievePersistedContext } from "./persisted.js";
import { ContextRetrievalGateway } from "./gateway.js";
...
export async function serveContextRetrievalMcp(): Promise<void> {
  ...
  for await (const line of rl) {
    ...
    try { write({ jsonrpc: "2.0", id: request.id, result: await handle(request, root, config, operationId, logicalAgent, phase) }); }
    catch (error) { write({ jsonrpc: "2.0", id: request.id, error: { code: -32603, message: error instanceof Error ? error.message : String(error) } }); }
  }
}

let gateway: ContextRetrievalGateway | undefined;
async function handle(request: { method?: string; params?: Record<string, unknown> }, root: string, config: Awaited<ReturnType<typeof loadProjectConfig>>, operationId: string, logicalAgent: string, phase: string): Promise<Record<string, unknown>> {
  ...
  gateway ??= await createPersistedContextGateway(root, config, operationId, logicalAgent, phase);
  const result = await retrievePersistedContext(root, config, operationId, logicalAgent, { fragmentId: args.fragmentId, section: args.section === "source" ? "source" : "raw", maxTokens: typeof args.maxTokens === "number" ? args.maxTokens : undefined }, phase, gateway);
