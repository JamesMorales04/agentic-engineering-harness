...
export const SERENA_HEADLESS_ARGS = ["--enable-web-dashboard", "false", "--open-web-dashboard", "false", "--enable-gui-log-window", "false"] as const;
...
export class SerenaSemanticProvider {
  ...
  async doctor(root: string): Promise<ProviderHealth> {
    ...
    const result = await runExecutable(this.command, ["--version"], { cwd: root, timeoutMs: 15_000 });
  ...
  mcpServer(root: string): McpServerConfig {
    return { description: "AEH-managed headless Serena semantic repository retrieval; editing is disabled by default.", type: "local", command: [this.command, "start-mcp-server", "--context", "ide-assistant", "--project", root, ...SERENA_HEADLESS_ARGS], enabled: true, timeoutMs: 30_000 };
  }
}
