...
export function profileTools(toolchain: ToolchainConfig, profile: string): string[] { const visiting = new Set<string>(); const result = new Set<string>(); const visit = (name: string): void => { if (visiting.has(name)) throw new Error(`Toolchain profile cycle detected at '${name}'.`); const value = toolchain.profiles?.[name]; if (!value) throw new Error(`Unknown toolchain profile '${name}'.`); visiting.add(name); for (const parent of value.extends ?? []) visit(parent); for (const tool of value.tools ?? []) result.add(tool); visiting.delete(name); }; visit(profile); return [...result]; }
...
async function addProjectCapabilities(root: string, result: Set<string>): Promise<void> { const files = new Set(await fs.readdir(root).catch(() => [] as string[])); if (files.has("package.json")) result.add("project:node"); if (files.has("bun.lock") || files.has("bun.lockb")) result.add("project:bun"); if (files.has("pnpm-lock.yaml")) result.add("project:pnpm"); if (files.has("yarn.lock")) result.add("project:yarn"); if (files.has("uv.lock") || files.has("pyproject.toml")) result.add("project:python"); if ([...files].some((name) => name.endsWith(".sln") || name.endsWith(".slnx") || name.endsWith(".csproj")) || files.has("global.json")) result.add("project:dotnet"); if (files.has("go.mod")) result.add("project:go"); if (files.has("Cargo.toml")) result.add("project:rust"); try { const pkg = JSON.parse(await fs.readFile(path.join(root, "package.json"), "utf8")) as { packageManager?: unknown }; const manager = typeof pkg.packageManager === "string" ? pkg.packageManager.split("@")[0] : undefined; if (manager) result.add(`project:${manager}`); } catch { /* optional */ } }
async function projectVersionOverrides(root: string): Promise<Record<string, string>> { const result: Record<string, string> = {}; for (const name of [".node-version", ".nvmrc"]) { try { const value = (await fs.readFile(path.join(root, name), "utf8")).trim().replace(/^v/, ""); if (value) { result.node = value; break; } } catch { /* optional */ } } try { const global = JSON.parse(await fs.readFile(path.join(root, "global.json"), "utf8")) as { sdk?: { version?: unknown } }; if (typeof global.sdk?.version === "string" && global.sdk.version.trim()) result.dotnet = global.sdk.version.trim(); } catch { /* optional */ } try { const pkg = JSON.parse(await fs.readFile(path.join(root, "package.json"), "utf8")) as { packageManager?: unknown }; if (typeof pkg.packageManager === "string") { const match = pkg.packageManager.match(/^bun@(.+)$/); if (match) result.bun = match[1]; } } catch { /* optional */ } return result; }
function expandDependencies(toolchain: ToolchainConfig, selected: Map<string, string[]>): void { let changed = true; while (changed) { changed = false; for (const name of [...selected.keys()]) { const tool = requireTool(toolchain, name); for (const dependency of tool.dependsOn ?? []) if (!selected.has(dependency)) { select(selected, dependency, `dependency:${name}`); changed = true; } } } }
