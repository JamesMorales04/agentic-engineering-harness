...
Mise is the default provisioning adapter because it can install tools from multiple ecosystems without making those tools npm dependencies of AEH.
...
AEH first tries an existing `mise` executable. If it is absent, explicit setup can bootstrap mise through `npm exec --yes -- mise`. This is an explicit setup action rather than an npm installation side effect.
